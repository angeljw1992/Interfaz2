...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.altaEmpleado.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.alta-empleados.update", [$altaEmpleado->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group mb-2">
                <label class="form-label required" for="id_empleado">{{ trans('cruds.altaEmpleado.fields.id_empleado') }}</label>
                <input class="form-control {{ $errors->has('id_empleado') ? 'is-invalid' : '' }}" type="number" name="id_empleado" id="id_empleado" value="{{ old('id_empleado', $altaEmpleado->id_empleado) }}" step="1" required>
            ...
            <div class="form-group mb-2">
                ...
                <input class="form-control {{ $errors->has('nombre_empleado') ? 'is-invalid' : '' }}" type="text" name="nombre_empleado" id="nombre_empleado" value="{{ old('nombre_empleado', $altaEmpleado->nombre_empleado) }}" required>
            ...
            <div class="form-group mb-2">
                ...
                <select class="form-control {{ $errors->has('role') ? 'is-invalid' : '' }}" name="role" id="role" required>
                    ...
                    @foreach(App\Models\AltaEmpleado::ROLE_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('role', $altaEmpleado->role) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
                @if($errors->has('role'))
            ...
            <div class="form-group mb-2">
                ...
                <select class="form-control select2 {{ $errors->has('local') ? 'is-invalid' : '' }}" name="local_id" id="local_id">
                    @foreach($locals as $id => $entry)
                        <option value="{{ $id }}" {{ (old('local_id') ? old('local_id') : $altaEmpleado->local->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
                @if($errors->has('local'))
